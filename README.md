...
> 🎓 **Учебный проект (ноябрь 2025).** Интернет-магазин «Ozon» за 4 дня на интенсиве GloAcademy. Вёрстка и логика написаны по ходу занятий, коммиты `Day 1`–`Day 4` соответствуют дням интенсива. Позже я настроила сборку, перевела данные на локальный JSON и написала этот README.
...
| Сборка | Webpack 5, webpack-dev-server, sass, html-loader |
...
> Фотографии товаров подгружаются с CDN Ozon (`cdn1.ozone.ru`). На момент съёмки CDN был недоступен из той сети, где делались скриншоты, поэтому карточки на них без картинок.
...
- **Внешние зависимости.** Bootstrap подключён целиком с CDN ради сетки, хотя хватило бы CSS Grid. Картинки товаров лежат на чужом CDN, и если он недоступен, у всех карточек пропадают фото.
